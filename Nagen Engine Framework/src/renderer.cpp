...
sf::Vector3f calculateNormal(const sf::Vector3f& p1, const sf::Vector3f& p2, const sf::Vector3f& p3) {
    ...
    for (auto &object : localScene->NagenObjectList) {
        ...
        for (const auto &polygon : object.polygons) {
            ...
            if (std::all_of(points.cbegin(), points.cend(), [](const sf::Vector2f& p) { return p != sf::Vector2f(-1.0f, -1.0f); })) {
                sf::Vector3f p1(polygon.first.xCoordinate, polygon.first.yCoordinate, polygon.first.zCoordinate);
                sf::Vector3f p2(polygon.second.xCoordinate, polygon.second.yCoordinate, polygon.second.zCoordinate);
                sf::Vector3f p3(polygon.third.xCoordinate, polygon.third.yCoordinate, polygon.third.zCoordinate);
                sf::Vector3f normal = calculateNormal(p1, p2, p3);

                sf::Vector3f viewDirection(player->x - object.xCoordinate, player->y - object.yCoordinate, player->z - object.zCoordinate);

                float dotProduct = normal.x * viewDirection.x + normal.y * viewDirection.y + normal.z * viewDirection.z;
